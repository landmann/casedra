...
- Resolver version `localiza-bootstrap-2026-04-23.21` tightens duplicate evidence and dossier links: indexed duplicate address signals now need multiple discriminating matches, user-facing report/candidate/download links keep only official/public ground-truth URLs or the exact submitted Idealista listing, and other portal or duplicate-listing URLs may remain stored as operator evidence but are not rendered as property links unless a licensed/source contract can prove same-property attribution.
- `/app/localiza` now uses the shared authenticated app top bar, matching `/app/inbox` and `/app/newsletter`.
- `/app/localiza` now includes a `Tabla` workspace: each user gets a durable table of their Localiza results, can sort by price, surface, price per m², rooms, confidence, or status, remove unwanted rows, import batches of Idealista links into the same table, and export the visible rows to CSV.
...
- The resolver version `localiza-bootstrap-2026-04-23.21` strips unsafe external listing links from generated dossiers and candidate cards, prevents public duplicate evidence from masquerading as an official candidate URL, requires indexed duplicate address hits to carry at least three discriminating listing-fact matches, and drops optional archive-provider payloads when the returned listing id or URL does not match the submitted Idealista listing.
- `/app/localiza` now persists a per-user table history in Convex through `localizaUserPropertyHistory`. The `Tabla` tab reads that history through tRPC, records every successful resolver call automatically, supports batch link import through the same resolver, hides removed rows per user, sorts the visible rows client-side, and exports the sorted table as CSV.
...
- [x] Decide the first `resolverVersion` format and bump policy.
      Current state: `apps/web/src/server/localiza/version.ts` exports `localiza-bootstrap-2026-04-23.21` and the `stable-bootstrap-date-plus-patch` policy.
